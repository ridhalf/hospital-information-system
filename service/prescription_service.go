...
type PrescriptionService interface {
	...
	Update(request web.PrescriptionUpdateRequest) (domain.Prescription, error)
}
...
func (service PrescriptionServiceImpl) Update(request web.PrescriptionUpdateRequest) (domain.Prescription, error) {
	count, err := service.userRepository.Count(request.DoctorID)
	if err != nil {
		return domain.Prescription{}, errors.New("an error occurred while processing your request")
	}
